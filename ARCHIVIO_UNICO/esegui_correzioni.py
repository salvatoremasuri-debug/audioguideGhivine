#!/usr/bin/env python3
"""Corregge SRT: pre_merge per casi speciali, verifica per tutti gli altri."""
...
LANG_FOLDERS = ["IT", "EN", "FR", "SP", "DE"]


def run_premerge() -> None:
  specs = []
  for (folder, num), rel in PREMERGE_MAP.items():
    out = OUTPUT / folder / "srt" / f"{num}.srt"
    if folder == "SP" and not (OUTPUT / "SP").exists():
      out = ARCHIVIO / "SP" / "srt" / f"{num}.srt"
...
def patch_verifica_for_output_folders() -> None:
  """Esegue verifica_e_correggi_srt su cartelle IT/EN/FR presenti in output."""
  ...
  for code, folder in folder_by_code.items():
    ...
    for srt_path in sorted(srt_dir.glob("*.srt")):
      if srt_path.stem.endswith("__styled_tmp"):
        continue
      key = (folder, srt_path.stem)
      if key in PREMERGE_MAP:
        continue

      txt_code = "ES" if code == "SP" else code
      txt_num = f"{int(srt_path.stem):02d}"
      txt_path = ARCHIVIO / "testi_originali" / txt_code / f"{txt_num}_{txt_code}.txt"
      ...
      too_long = any(
        len(re.sub(r"\s+", " ", t).strip()) > 90
        or any(len(ln) > 16 for ln in re.split(r"\r?\n", t))
      ...
      wrapped = vec.rebuild_from_original(cues, orig_text, 90, 16, 9, 14)
      srt_path.write_text(vec.build_srt(wrapped), encoding="utf-8")
      fixed.append(srt_path.relative_to(OUTPUT).as_posix())
      print(f"verifica: {srt_path.name} ({folder})")

  out = ARCHIVIO / "fixed_verifica_output.txt"
...
def main() -> None:
  ...
  print("=== 1/3 Rifacimento da pre_merge ===")
  run_premerge()
  print("=== 2/3 Verifica e correzione schermate ===")
  patch_verifica_for_output_folders()
  print("=== 3/3 Normalizzazione finale ===")
  run_normalizza()
